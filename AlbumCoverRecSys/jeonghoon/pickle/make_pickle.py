...
z = tf.placeholder(dtype=tf.string, shape=[None])
...
def read_and_decode(filename_queue):
    ...
    filenames = tf.cast(features["image/filename"], tf.string)
    ...
    return image_, label_, filenames

filename_queue = tf.train.string_input_producer(tfrecord_auto_traversal(), num_epochs=1)

image, label, names = read_and_decode(filename_queue)
...
label_value = tf.reshape(tf.cast(tf.argmax(y_label, 1), dtype=tf.int32), shape=[64])
max_point = tf.reshape(tf.cast(tf.argmax(y_vgg, 1), dtype=tf.int32), shape=[64])
...
z_ = tf.cast(z, dtype=tf.string)

example_batch, label_batch, name_batch = tf.train.shuffle_batch([image, label, names], batch_size=64, num_threads=8,
...
with tf.Session() as sess:
    ...
    saver.restore(sess, '/data/genre_ckpts/genre_Ckpt-1')
    coord = tf.train.Coordinator()
    threads = tf.train.start_queue_runners(sess=sess, coord=coord)
    merge = tf.summary.merge_all()
    #train_writer = tf.summary.FileWriter('./genre_summary/', sess.graph)
    now_epoch = 0
    f1 = open("./value_pickle.txt", 'wb') # pickle file open
    f2 = open("./name_pickle.txt", 'wb')

    for i in range(200):
        #import pdb;pdb.set_trace()
        batch_x, batch_y, batch_z = sess.run([example_batch, label_batch, name_batch])
        allvalue, result, names_ = sess.run([y_vgg, max_point, z_], feed_dict={keep_prob: 1.0, x: batch_x, y: batch_y, z: batch_z, task: False})
        #allvalue, result, _name = sess.run([y_vgg, max_point, z], feed_dict={x: batch_x, y: batch_y, z: batch_z, task: True, keep_prob: 1.0})
        #batch_x, batch_y= sess.run([example_batch, label_batch]) 
        #y_value, result = sess.run([y_vgg, max_point], feed_dict={x: batch_x, y: batch_y, task: False, keep_prob: 1.0})
        #for_value = np.array([])
        #for_name = np.array([])
        # _name.astype(str)
        # import pdb;pdb.set_trace()
        #batch_z = [int(x.strip('.jpg')) for x in batch_z.astype(str)]
        _name2 = names_
        _name2 = [int(r.strip('.jpg')) for r in _name2.astype(str)]
        for u in range(64):
            pickle.dump(allvalue[u,], f1)
            pickle.dump(_name2[u], f2)
        print("%d ====================" % i)
        ...
        '''if i % 100 != 0:
            ...
            print(t.strftime("%Y-%m-%d %H:%M:%S", cusrrentTime))
